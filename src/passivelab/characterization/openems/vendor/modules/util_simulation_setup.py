...
def runSimulation (excite_portnumbers, FDTD, sim_path, model_basename, preview_only, postprocess_only, force_simulation=False, interactive_preview=False, num_threads=None, verbose=None, dump_statistics=False):
    ...
    if not postprocess_only:
        ...
        # preview model (PassiveLAB 1.4.1: gated on an explicit opt-in flag, not on
        # `1 in excite_portnumbers`, so batch/headless/Ray callers never hit a blocking GUI
        # launch by default -- the golden notebook worked around this with a dead-branch
        # `if False and 1 in excite_portnumbers:` edit; this is the same effect done cleanly)
        if interactive_preview and 1 in excite_portnumbers:  # only for first port excitation
