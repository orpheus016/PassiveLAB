...
Importing this package self-registers ``TCoilLayoutGenerator`` (and, since 1.3.3, ``TCoilSpec``)
into the core registry under ``passive_type="tcoil"`` (sub-phase 1.3.1) — the standard "importing
the plugin registers it" pattern, so a caller who imports ``passivelab.geometry.tcoil`` to reach
``TCoilSpec`` gets dispatch through ``passivelab.core.generate(spec)`` and
``passivelab.core.load_spec("spec.json")`` for free, with no explicit registration call.
"""
from passivelab.core.geometry.registry import register, register_spec
